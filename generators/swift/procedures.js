...
Blockly.Swift['procedures_defreturn'] = function(block) {
  ...
  if (Blockly.Swift.STATEMENT_PREFIX) {
    ...
    branch = Blockly.Swift.prefixLines(
        ...
        '\'' + id + '\''), Blockly.Swift.INDENT) + branch;
  }
  ...
  if (returnValue) {
    returnValue = Blockly.Swift.INDENT + 'return ' + returnValue + ';\n';
  }
  var args = [];
  for (var i = 0; i < block.arguments_.length; i++) {
    args[i] = Blockly.Swift.variableDB_.getName(block.arguments_[i],
...
Blockly.Swift['procedures_callreturn'] = function(block) {
  ...
  var args = [];
  for (var i = 0; i < block.arguments_.length; i++) {
    args[i] = Blockly.Swift.valueToCode(block, 'ARG' + i,
...
Blockly.Swift['procedures_ifreturn'] = function(block) {
  ...
  if (block.hasReturnValue_) {
    ...
    code += Blockly.Swift.INDENT + 'return ' + value + ';\n';
  } else {
    code += Blockly.Swift.INDENT + 'return;\n';
  }
  code += '}\n';
  return code;
};
