...
Blockly.Swift.ORDER_NEW = 1.1;            // new
Blockly.Swift.ORDER_MEMBER = 1.2;         // . []
Blockly.Swift.ORDER_FUNCTION_CALL = 2;    // ()
Blockly.Swift.ORDER_INCREMENT = 3;        // ++
Blockly.Swift.ORDER_DECREMENT = 3;        // --
Blockly.Swift.ORDER_BITWISE_NOT = 4.1;    // ~
Blockly.Swift.ORDER_UNARY_PLUS = 4.2;     // +
Blockly.Swift.ORDER_UNARY_NEGATION = 4.3; // -
Blockly.Swift.ORDER_LOGICAL_NOT = 4.4;    // !
Blockly.Swift.ORDER_TYPEOF = 4.5;         // typeof
Blockly.Swift.ORDER_VOID = 4.6;           // void
Blockly.Swift.ORDER_DELETE = 4.7;         // delete
Blockly.Swift.ORDER_AWAIT = 4.8;          // await
Blockly.Swift.ORDER_EXPONENTIATION = 5.0; // **
Blockly.Swift.ORDER_MULTIPLICATION = 5.1; // *
Blockly.Swift.ORDER_DIVISION = 5.2;       // /
Blockly.Swift.ORDER_MODULUS = 5.3;        // %
Blockly.Swift.ORDER_SUBTRACTION = 6.1;    // -
Blockly.Swift.ORDER_ADDITION = 6.2;       // +
...
Blockly.Swift.ORDER_YIELD = 16.5;         // yield
 ...
 */
Blockly.Swift.ORDER_OVERRIDES = [
  // (foo()).bar -> foo().bar
  // (foo())[0] -> foo()[0]
  [Blockly.Swift.ORDER_FUNCTION_CALL, Blockly.Swift.ORDER_MEMBER],
  // (foo())() -> foo()()
  [Blockly.Swift.ORDER_FUNCTION_CALL, Blockly.Swift.ORDER_FUNCTION_CALL],
  // (foo.bar).baz -> foo.bar.baz
  // (foo.bar)[0] -> foo.bar[0]
  // (foo[0]).bar -> foo[0].bar
  // (foo[0])[1] -> foo[0][1]
  [Blockly.Swift.ORDER_MEMBER, Blockly.Swift.ORDER_MEMBER],
  // (foo.bar)() -> foo.bar()
  // (foo[0])() -> foo[0]()
  [Blockly.Swift.ORDER_MEMBER, Blockly.Swift.ORDER_FUNCTION_CALL],

  // !(!foo) -> !!foo
  [Blockly.Swift.ORDER_LOGICAL_NOT, Blockly.Swift.ORDER_LOGICAL_NOT],
  // a * (b * c) -> a * b * c
  [Blockly.Swift.ORDER_MULTIPLICATION, Blockly.Swift.ORDER_MULTIPLICATION],
  // a + (b + c) -> a + b + c
  [Blockly.Swift.ORDER_ADDITION, Blockly.Swift.ORDER_ADDITION],
  // a && (b && c) -> a && b && c
  [Blockly.Swift.ORDER_LOGICAL_AND, Blockly.Swift.ORDER_LOGICAL_AND],
  // a || (b || c) -> a || b || c
  [Blockly.Swift.ORDER_LOGICAL_OR, Blockly.Swift.ORDER_LOGICAL_OR]
];
...
Blockly.Swift.init = function(workspace) {
  ...
  Blockly.Swift.variableDB_.setVariableMap(workspace.getVariableMap());

  var defvars = [];
  // Add developer variables (not created or named by the user).
  var devVarList = Blockly.Variables.allDeveloperVariables(workspace);
  for (var i = 0; i < devVarList.length; i++) {
    defvars.push(Blockly.Swift.variableDB_.getName(devVarList[i],
  ...
  for (var i = 0; i < variables.length; i++) {
    defvars.push(Blockly.Swift.variableDB_.getName(variables[i].getId(),
...
Blockly.Swift.scrub_ = function(block, code) {
  ...
  if (!block.outputConnection || !block.outputConnection.targetConnection) {
    ...
    comment = Blockly.utils.wrap(comment, Blockly.Swift.COMMENT_WRAP - 3);
    if (comment) {
      if (block.getProcedureDef) {
        // Use a comment block for function comments.
        commentCode += '/**\n' +
                       Blockly.Swift.prefixLines(comment + '\n', ' * ') +
                       ' */\n';
      } else {
 ...
 */
Blockly.Swift.getAdjusted = function(block, atId, opt_delta, opt_negate,
    opt_order) {
  var delta = opt_delta || 0;
  var order = opt_order || Blockly.Swift.ORDER_NONE;
  if (block.workspace.options.oneBasedIndex) {
    delta--;
  }
  var defaultAtIndex = block.workspace.options.oneBasedIndex ? '1' : '0';
  if (delta > 0) {
    var at = Blockly.Swift.valueToCode(block, atId,
        Blockly.Swift.ORDER_ADDITION) || defaultAtIndex;
  } else if (delta < 0) {
    var at = Blockly.Swift.valueToCode(block, atId,
        Blockly.Swift.ORDER_SUBTRACTION) || defaultAtIndex;
  } else if (opt_negate) {
    var at = Blockly.Swift.valueToCode(block, atId,
        Blockly.Swift.ORDER_UNARY_NEGATION) || defaultAtIndex;
  } else {
    var at = Blockly.Swift.valueToCode(block, atId, order) ||
        defaultAtIndex;
  }
  ...
  if (Blockly.isNumber(at)) {
    ...
    if (delta > 0) {
      at = at + ' + ' + delta;
      var innerOrder = Blockly.Swift.ORDER_ADDITION;
    } else if (delta < 0) {
      at = at + ' - ' + -delta;
      var innerOrder = Blockly.Swift.ORDER_SUBTRACTION;
    }
    if (opt_negate) {
      if (delta) {
        at = '-(' + at + ')';
      ...
      var innerOrder = Blockly.Swift.ORDER_UNARY_NEGATION;
    }
